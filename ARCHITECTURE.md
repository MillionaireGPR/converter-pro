...
**Arquivos**: `main.py` (`_HEAVY_SLOT`, `_job_slot`, `MAX_CONCURRENT_JOBS`, `_auto_max_concurrent_jobs`)
**Por que existe**: Render Starter = 512MB. Jobs pesados (extração de IMAGEM — renderiza páginas em bitmap — e extração de IA) rodando em paralelo estouram a RAM → Render reinicia a instância → jobs em andamento morrem EM SILÊNCIO (comprovado: 6 catálogos simultâneos → 3 sem imagem + 1 travado + e-mail "exceeded its memory limit"). Um semáforo GLOBAL compartilhado pelos dois tipos de job limita a `MAX_CONCURRENT_JOBS` simultâneos; o resto fica "na fila" (status=processing, stage=queued — compatível com o polling).

**Autodetecção por RAM (13/08/2026)**: em vez de um valor fixo, `MAX_CONCURRENT_JOBS` se autoajusta pela memória REAL medida via `psutil` no boot (`_auto_max_concurrent_jobs`) — `>= 2048MB` libera 3 jobs em paralelo, abaixo disso força 1. Motivação: com a migração pra um servidor próprio (7.7GB) rodando em paralelo ao Render (fallback manual via troca de `VITE_BACKEND_URL` no Vercel), o MESMO código roda ora num ambiente com RAM sobrando, ora de volta no Render — configurar `MAX_CONCURRENT_JOBS` manualmente exigia lembrar de trocar toda vez que o fallback entrava/saía. Com a autodetecção, o próprio processo mede sua RAM e se protege sozinho, mesmo no Render. Env `MAX_CONCURRENT_JOBS` continua existindo, mas agora como OVERRIDE explícito — se setado, vence a autodetecção. NÃO remover nem trocar por execução paralela sem essa proteção por RAM (ou upgrade real que justifique um valor fixo maior).
