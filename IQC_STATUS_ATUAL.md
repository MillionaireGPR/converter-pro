...
- **Supabase Storage no limite (#167, achado pelo usuário 22/09):** bucket grátis de 1GB em 1,71GB (alerta "Organization exceeded its quota", corte previsto 21/10) — cada job de imagens deixava um ZIP permanente no bucket, sem nenhuma limpeza. Fix: `cleanup_old_storage_files` apaga ZIP com mais de 10 dias (`STORAGE_RETENTION_DAYS`), rodando sozinho 1x/dia numa thread de fundo desde o boot + endpoint manual `POST /admin/storage/cleanup`. Rodado no próprio deploy: 106 arquivos no bucket, 90 apagados, 695,8MB liberados. No ar e verificado no container.
