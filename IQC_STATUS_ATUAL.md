...
- **VAESO (#160):** as tabelas extras eram descartadas entre a conversão e a
  ...
  em `processarArquivoV2` (engine.ts, formato de compatibilidade). Corrigido nos dois; migration
  `20260918_standardized_products_mercos_extras.sql` **já aplicada** (coluna conferida no banco em 23/09).
...
- **PETRIN preço DE/POR (#166, retestagem 22/09):** RD1602 saía com R$35 (preço do vizinho RD1604) em vez de "DE R$16 POR R$8"; o par DE/POR certo ia pro RD1098-1, bem mais longe na página — a IA seguia a ordem de leitura do texto, não a posição. Fix genérico: `_fix_labeled_promo_price` mede a posição real dos rótulos "DE"/"POR" no PDF e atribui ao código geometricamente mais próximo. Medido no catálogo real (800 produtos): 1 mudança (RD1602 corrigido), 0 mudanças no DUTE. No ar e verificado no container.
- **Retestagem PETRIN/DUTE (22/09) — demais itens:** RD1716, RD1582, RD1820, RD1098-1, DT10317 e os 31 códigos sumidos do DUTE já saem certos numa extração nova (sem mudança de código necessária). Os "EM BREVE" falsos e preços fantasma no PETRIN não reproduziram — é ruído de percepção da IA de Visão num selo visual (sem texto), não achado determinístico pra corrigir. **Ainda aberto:** composição de foto do DUTE absorvendo imagem de produto vizinho (ex.: DT10421 pág. 17 puxando a foto de um "LABIRINTO DE BOLINHAS" que não é dela) — causa raiz achada (`_dute_axis_partitions` divide linhas por PONTO MÉDIO entre centros de SKU, ignorando a linha de grade real detectada no PDF), mas fix ainda não escrito; PETRIN tem 44 fotos trocadas com vizinho e 13 sem foto (8 batem com os 9 que o Josef já sabia que não tem foto no catálogo, 5 novos não investigados) ainda pendentes de investigação.
- **Supabase Storage no limite (#166/#167, achado pelo usuário 22/09):** bucket grátis de 1GB em 1,71GB (alerta "Organization exceeded its quota", corte previsto 21/10) — cada job de imagens deixava um ZIP permanente no bucket, sem nenhuma limpeza. Fix: `cleanup_old_storage_files` apaga ZIP mais antigo que `STORAGE_RETENTION_DAYS` (padrão **3 dias**, pedido do usuário), rodando sozinho 1x/dia numa thread de fundo desde o boot + endpoint manual `POST /admin/storage/cleanup`. Bucket AO VIVO caiu de 295,6MB → **78,4MB / 1GB (7,8%)**. **Importante:** o painel do Supabase pode continuar mostrando a cota estourada por mais alguns dias mesmo com o bucket vazio — "Storage Size" ali é MÉDIA do ciclo de faturamento (GB-Hrs), não o tamanho atual; cai sozinha conforme os dias com armazenamento baixo se acumulam. No ar e verificado no container.
- **Base cortada em 1000 produtos (#168, 23/09):** depois de recarregar a página, Base Padronizada / Exportação Mercos / Dashboard só enxergavam 1000 dos 6275 produtos (limite por consulta do Supabase) — produto podia "sumir" da exportação sem erro. Carga agora paginada (6275/6275). Contador de produtos por fornecedor somava a cada conversão (MOMENT 102.042 → real 1.484): passa a gravar a contagem real, e os 32 fornecedores foram corrigidos no banco. Removidos da tela: botão "Seed Dados Base", sino de notificação fixo, busca do topo sem ação e a página Configurações (dados fictícios, Salvar não salvava).
- **Fornecedor novo sem depender do nome (#169, 23/09):** preparado pro recadastro dos fornecedores. FOLIA (cards sem texto), FORTAL (preço "UND:") e GOAL KIDS (prefixo "GK") agora são reconhecidos pelo formato do arquivo, com qualquer nome. DUTE e DAGIA viraram 2 opções no cadastro do fornecedor (tela de upload e Editar): **"Foto do produto é montada por várias imagens"** e **"IA escolhe a foto de cada produto"** (a medição mostrou que o automático erraria entre DUTE e PETRIN). Prova sem gastar API: 9 catálogos reais com nome neutro geram exatamente as mesmas fotos (MD5) que o código anterior com o nome real. Ainda dependem do nome: dicas de extração de 12 fornecedores (sem elas entra a auto-análise + Particularidades) e os leitores de planilha (nome novo = leitor genérico + Conferência de colunas).
...
- **Folia:** o PDF é uma arte visual, sem código/nome/preço pesquisáveis. A
  ...
  recebem uma segunda leitura seletiva e a foto é associada diretamente ao
  cartão visual mais próximo. No arquivo real de 20/07/2026: **367 produtos
  únicos, 367 imagens associadas, 0 sem imagem e 0 recortes minúsculos**.
- **Fortal:** quando o cartão mostra `UND: R$ 7,20` e abaixo `R$ 72,00`, o
  segundo valor é o total da caixa. Um pós-processamento por coordenada agora
  escolhe o valor `UND:` do mesmo SKU. No resultado real com 949 produtos,
  havia 82 rótulos unitários e **81 preços de caixa foram corrigidos**; por
  exemplo, `BDZ-2523` passou de `72,00` para `7,20`.
