...
| #171 | **Retestagem do Josef 24/09 (BM36 fechado)**: FOLIA JRF-50.0111/0020/0113 (pág. 12) sem foto; PETRIN RD1020 (EM BREVE) com o R$ 2,20 do RD1021 e RD1098-1 (EM BREVE) com o POR R$ 8,00 + PROMOÇÃO do RD1602 | FOLIA: `_costurar_tiles` juntava 3 cards encostados de 190/190/192pt (diferença >2pt) numa foto só → na grade de cards não costura mais. PETRIN: código SEM nenhum preço na região do card perde o preço da IA (normal ou promocional) quando ele está impresso na região de OUTRO código — antes era poupado se o vizinho tinha o mesmo valor ou se o card vizinho tinha 2 preços (DE/POR) | FOLIA com posições reais: 300/300 fotos, só as 3 novas. PETRIN: 25 preços zerados — os 2 do Josef + 23 latentes, os 25 conferidos na imagem da página, todos com selo EM BREVE; outros 8 catálogos 0 mudanças. 69 testes backend. Ver `guide.md #14.34` |
| #172 | **Retestagem do Josef 24/09 (DUTE recadastrada + Neo Festas nova)**: DUTE DT10421/WD0115/DT10231 com pedaço do vizinho; FOLIA pág. 15 (9 trocadas), pág. 21 (2 trocadas), pág. 11 código lido errado; PETRIN 25 ativos com ***EM BREVE*** e 8 sem foto; Neo Festas 479 nomes = texto de preço, preço do vizinho, 30 cores sumindo, duplicados, cor da bolinha não aplicada | Neo: `_template_desalinhado` (preço ANTES do código medido nas páginas / nome = preço) derruba o template → IA lê o catálogo inteiro; `_nomear_cores_por_bolinha` lê a cor do círculo vetorial acima de cada código. FOLIA: `_folia_cards_por_ordem` (a visão erra a escala, não a ordem) + `_conferir_codigos_por_card` (relê o código no recorte 300dpi do card, troca só com mesmo nome/formato). PETRIN: selo EM BREVE destrocado na conferência por geometria (N marcados com preço no card × N sem marcação sem preço); orientação 'abaixo' no casamento embedded + passada 'foto logo abaixo do código'. DUTE: `_dute_dono_de_cada_imagem` (código acima-à-esquerda, vão H + 2×vão V) no lugar da faixa linha×coluna | Neo (rodada real no servidor): faltando 10→0, duplicados 11→0, nome=preço 489→0, sem nome 43→0, amostra de preço 8/8 certa; 240 cores. FOLIA: 23 fotos corrigidas conferidas uma a uma; 0040/0365 corrigidos. PETRIN: 25/25 EM BREVE, 50 selos destrocados, 9 fotos a mais (RD1333 fica). DUTE 650: 189 fotos mudam, revisadas lado a lado — barra de rodapé saiu de ~10 fotos, peças de vizinho saíram. Outros catálogos: 0 fotos mudam. 80 testes backend. Ver `guide.md #14.35` |
| #173 | **Retestagem do Josef 25/09**: DUTE 9 códigos com linha tracejada/texto da ficha/rodapé dentro da foto; PETRIN RD1333 sem foto e RD1715 com a estante de outro produto; Neo Festas págs. 6/13 com fotos trocadas e sem foto, 152870 sem foto, 9 códigos de cor sumidos da exportação; FOLIA JRF-50.0189 "ABRIOR" | DUTE: `_render_so_imagens` (cópia descartável da página sem texto/vetor/outras imagens) na foto composta e no fallback + `_icones_de_caracteristica` (imagem pequena repetida 3+ vezes no catálogo sai da composição). PETRIN: `_retangulo_visivel` (foto recortada por clip usa a parte visível — não derruba mais a do vizinho como "selo") + bloco do código até o próximo código de qualquer coluna. Neo: busca nas colunas vizinhas soma distância horizontal; códigos da mesma coluna a ≤40pt dividem a foto do card; `_limpar_marcador_do_codigo` ("104736*"); `_conferir_codigos_pelo_texto` (código que não existe no texto do PDF × código órfão com o nome logo antes). FOLIA: releitura do card corrige 1 letra de 1 palavra do nome | DUTE 600/632 fotos mais limpas (sem texto/linha/fundo/ícone); PETRIN 66 fotos mudam (revisadas; RD1333, RD1113, RD1984 ganham foto); Neo sem "IA escolhe a foto": 2162 de 2245 com foto (IA dava 1440), págs. 6/9/13/67 conferidas código a código; BM36, FORTAL, GIRA, VAESO, FOLIA 0 mudanças. Ver `guide.md #14.36`. Testes: 13 novos (`test_rodada_2509.py`), todos falham no código antigo; 93 no backend |
| #174 | **Retestagem do Josef 25/09 (tarde)**: Neo MINI FLOR ROSA EVA/ARTIFICIAL e GLITTER GEL com foto de cor errada/repetida; VARETA (10 códigos) com a foto da tela triangular, 148296/150614 sem foto, 161292 com a bomba; DUTE DT10176 com triângulo do rodapé | `_variantes_em_grade` (grade de fotos de cor ao lado da lista de códigos → casa por ordem); `_juntar_pecas_finas` + `_montar_pecas` (imagens <20pt encostadas viram uma foto; cada peça pode ir pra um código); SMask em PNG cinza; `_transparente_sob` (imagem da página sobre parte transparente da foto sai) | Neo (catálogo inteiro, sem IA escolhendo foto): 2169→2210 com foto; mudanças revisadas lado a lado (cores de MINI FLOR/GLITTER/GRAVATA/COLAR/SAIA/TIARA/BOLA ESPELHADA/JOGO AMERICANO certas, varetas por cor). DUTE 12 fotos mudam (rodapé/ícone fora). PETRIN 3. BM36, FORTAL, GIRA, VAESO, DAGIA, FOLIA 0. 20 testes em `test_rodada_2509.py`, 100 no backend. Ver `guide.md #14.37` |
| #175 | **Neo: manter o padrão ***CORES*** (decisão Gabriel 25/09)** — regra do cadastro virava "CORES" sem asteriscos e depois nome de cor | `_COMPILE_RULES_PROMPT` preserva marcação literal no fim do nome (nunca no código); `_nomear_cores_por_bolinha` respeita marcação ***X*** do cliente; `rules_hash` v2 recompila o cache | Compilação real no servidor: "Adicionar ***CORES*** ao final do campo nome" (2/2). 101 testes |
